...
hosp_boxplot <- hospitalisation_MIQR %>%
  ...
  scale_y_continuous(name="Rate per 100,000",trans = "log",
                     ...
                     limits = c(min(hospitalisation_MIQR$Mean)/3,
                                max(hospitalisation_MIQR$Mean)*1.4), labels = scales::comma) + 
...
mort_boxplot<- mortality_MIQR  %>%
  ...
  scale_y_continuous(name="Rate per 100,000",trans = "log",
                     ...
                     limits = c(min(mortality_MIQR $Mean)/3,
                                max(mortality_MIQR $Mean)*1.4), labels = scales::comma) + # labels = comma 
  scale_x_discrete(name = "Age group") +
...
mode_obse_boxplot <- mode_obse_MIQR %>%
  ...
  geom_boxplot(width=0.25,alpha=0.35,aes(colour="blue4",fill="blue4")) + 
  geom_text(y =-5.4, aes(label = num),size=3,colour="blue4") + 
  scale_y_continuous(name="Recorded/Modelled",trans = "log",
                    #  breaks = c(0.01,0.05,0.1,0.25,0.7,1,6.25),
                     breaks = c(0.01,0.05,0.1,0.25,1,6.25),
                     limits = c(0.005,
...
# 1.3.1 QA----
mode_obse_MIQR_QA <- mode_obse_MIQR[mode_obse_MIQR$QA>3,] # all high quality
...
# 4.1 formap----
# load mapdata
world_map <- readOGR(dsn = "Worldmap/World_Countries.shp", stringsAsFactors = TRUE)
x <- world_map@data 
