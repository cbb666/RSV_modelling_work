genHospRate <- function(eachAgeDefIncome,choice=NULL){  
  # eachAgeDefIncome <- ARI_hospitalisation_meta[ARI_hospitalisation_meta$Agegr=="0-125" & ARI_hospitalisation_meta$income=="High income",]
...
genMorRate <- function(eachAgeDefIncome,choice=NULL){  
  # eachAgeDefIncome <- all_cause_mortality_meta[all_cause_mortality_meta$Agegr=="5-17" & all_cause_mortality_meta$income=="Upper middle income",]
  if(length(unique(eachAgeDefIncome$Study))<2){
    return(NULL)
  }else{
    message(eachAgeDefIncome$Case_definition[1],eachAgeDefIncome$Agegr[1],eachAgeDefIncome$income[1])
    ...
    if(choice %in% c("65-125","75-125")){
      ...
      forest(fit_eachAgeDefIncome_meta, 
             ...
             # alim=c(-4,10),
             ylim=c(-1,max(rows)+3),
             at = log(c(0.1,0.5,2.5,12.5,62.5)), # 修改
             # log(max(all_cause_mortality_meta$`upper CI mean`));log(max(all_cause_mortality_meta$`lower CI mean revised`)) 
             atransf = exp,
             refline =coef(fit_eachAgeDefIncome_meta),
             header=FALSE,
